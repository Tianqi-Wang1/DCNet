...
class CompLoss(nn.Module):
    ...
    '''
    def __init__(self, P, temperature=0.07, base_temperature=0.07):
    ...
    def forward(self, features, prototypes, labels):
        ...
        feature_in = features
        feature_in = F.normalize(feature_in, dim=1)
        proxy_labels = torch.arange(0, self.P.n_cls_per_task*4).cuda()
        labels_in = labels.contiguous().view(-1, 1)
        mask = torch.eq(labels_in, proxy_labels.T).float().cuda() #bz, cls

        # compute logits
        feat_dot_prototype = torch.div(
            torch.matmul(feature_in, prototypes.T),
            self.temperature)
        # for numerical stability
